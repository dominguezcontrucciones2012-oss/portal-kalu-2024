...
if (!fs.existsSync(serviceAccountPath)) {
  ...
  console.error(`Por favor, descarga tu llave privada desde Firebase Console (Configuración del Proyecto > Cuentas de Servicio),`);
  console.error(`guárdala en esta carpeta con el nombre "service-account.json" y vuelve a intentarlo.`);
  process.exit(1);
}
...
async function runMigration() {
  ...
  for (const doc of usersSnapshot.docs) {
    ...
    // Si el usuario no tiene PIN de 4 dígitos o cédula, no podemos migrarlo fácilmente de esta manera.
      ...
      try {
        ...
        if (err.code === 'auth/user-not-found') {
          ...
          authRecord = await auth.createUser({
            uid: doc.id, // Reutilizar el mismo UID para mantener los vínculos en la BD
            email: emailToUse,
            password: newPin, // Usamos el PIN extendido como clave
