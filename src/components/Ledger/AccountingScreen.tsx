...
const AccountingScreen: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const unsubCompras = subscribeToCollection('compras_mercancia', (data) => setCompras(data));
    const unsubProv = subscribeToCollection('providers', (data) => setProviders(data));
  ...
  const PLAN_CUENTAS: CuentaContable[] = [
    ...
    { id: '2.1.02', codigo: '2.1.02', nombre: 'Cuentas por Pagar Proveedores', tipo: 'Pasivo', naturaleza: 'Acreedora', es_detalle: true },
    ...
    { id: '5.1.02', codigo: '5.1.02', nombre: 'Costo de Venta (Mercancía Gral)', tipo: 'Egreso', naturaleza: 'Deudora', es_detalle: true },
  ];
  ...
  // 3. Procesar Compras de Mercancía
  compras.forEach((compra) => {
    const details: any[] = [];
    const rate = tasaBcv;
    const montoUsd = compra.total_usd || 0;
    const montoBs = parseFloat((montoUsd * rate).toFixed(2));
    ...
    if (compra.es_credito) {
      ...
      details.push({
        cuenta_id: '1.1.01', // Caja USD (se paga de contado)
        debe_usd: 0,
        haber_usd: montoUsd,
        debe_bs: 0,
        haber_bs: montoBs
    ...
    derivedAsientos.push({
      id: `compra-${compra.id}`,
      fecha: compra.fecha,
      descripcion: `COMPRA MERCANCÍA: ${compra.proveedor_nombre || 'Proveedor'}`,
      tasa_referencia: rate,
      detalles: details
    });
  });
  ...
  const getAccountBalance = (cuentaId: string) => {
    ...
    derivedAsientos.forEach(asiento => {
      asiento.detalles.forEach(det => {
        if (det.cuenta_id === cuentaId) {
          if (cuentaId === '1.1.01' || cuentaId === '1.1.02' || cuentaId === '1.1.03' || cuentaId === '5.1.01' || cuentaId === '5.1.02') {
    ...
    if (cuentaId === '2.1.02') {
      // Cuentas por pagar a proveedores en tiempo real
      return providers.reduce((sum, p) => sum + (p.saldo_usd || 0), 0);
    }
  ...
  const pasivos = getAccountBalance('2.1.01') + getAccountBalance('2.1.02');
