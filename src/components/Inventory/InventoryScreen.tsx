...
const InventoryScreen: React.FC = () => {
  ...
  useEffect(() => {
    let unsub = subscribeToCollection('products', (data) => {
      setProducts(data);
      setLoading(false);
    });
    ...
    const fetchProductsViaRest = async () => {
      try {
        ...
        const url = `https://firestore.googleapis.com/v1/projects/${projectId}/databases/${dbId}/documents/products?pageSize=200`;
        ...
        if (res.ok) {
          ...
          if (json.documents && json.documents.length > 0) {
            const docs = json.documents.map((d: any) => {
              ...
              const piezas: PiezaProducto[] = [];
              if (fields.piezas?.arrayValue?.values) {
                fields.piezas.arrayValue.values.forEach((v: any) => {
                  const pzFields = v.mapValue?.fields;
                  if (pzFields) {
                    piezas.push({
                      id: pzFields.id?.stringValue || '',
                      numero: pzFields.numero?.stringValue || '',
                      peso: parseNumber(pzFields.peso),
                      precio_usd: pzFields.precio_usd ? parseNumber(pzFields.precio_usd) : undefined,
                      vendida: pzFields.vendida?.booleanValue || false
                    });
                  }
                });
              }
              ...
              return {
                id: d.name?.split('/').pop() || '',
                codigo: fields.codigo?.stringValue || '',
                nombre: fields.nombre?.stringValue || '',
                categoria: fields.categoria?.stringValue || 'GENERAL',
                costo_usd: parseNumber(fields.costo_usd),
                precio_normal_usd: parseNumber(fields.precio_normal_usd),
                precio_oferta_usd: parseNumber(fields.precio_oferta_usd),
                margen_ganancia: parseNumber(fields.margen_ganancia),
                stock: parseNumber(fields.stock),
                stock_minimo: parseNumber(fields.stock_minimo),
                unidad_medida: fields.unidad_medida?.stringValue || 'UND',
                vendedor_id: fields.vendedor_id?.stringValue,
                estatus: fields.estatus?.stringValue as any,
                imagen_url: fields.imagen_url?.stringValue,
                imagen_secundaria_url: fields.imagen_secundaria_url?.stringValue,
                descripcion: fields.descripcion?.stringValue,
                usa_piezas: fields.usa_piezas?.booleanValue || false,
                piezas: piezas,
              } as Product;
            });
            setProducts(docs);
            setLoading(false);
            localStorage.setItem('kalu_products_data', JSON.stringify(docs));
          }
        }
      } catch (e) {
        console.warn('REST products fetch falló:', e);
      }
    };
