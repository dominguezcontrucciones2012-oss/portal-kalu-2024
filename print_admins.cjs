...
async function run() {
  try {
    const users = await queryFirestore('kalu-queso-sanjuam', 'administradores');
    console.log("TOTAL ADMINS:", users.documents ? users.documents.length : 0);
    if (users.documents) {
      users.documents.forEach(doc => {
        console.log(`Document ID: ${doc.name.split('/').pop()}`);
